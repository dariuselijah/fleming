...
import { checkFamilyEligibility } from "@/lib/medikredit/eligibility-service"
...
export async function POST(req: Request) {
  ...
  let body: { practiceId?: string; patient?: MedikreditPatientPayload; dependents?: MedikreditPatientPayload[]; persist?: boolean }
  try {
    body = await req.json()
  } catch {
  ...
  try {
    const result = await checkFamilyEligibility(patient, provider, body.dependents)
    ...
    console.error("[medikredit/family]", e)
    return NextResponse.json(
      { error: e instanceof Error ? e.message : "Family eligibility check failed" },
      { status: 502 }
    )
  }
}
